...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self.dataSource respondsToSelector:@selector(pickerView:modelForItem:)]) {
        ...
        cell.backgroundViewWidth         = model.backgroundWidth;
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self.dataSource respondsToSelector:@selector(pickerView:modelForItem:)]) {
        AKPickerItemModel *itemModel = [self.dataSource pickerView:self modelForItem:indexPath.item];
        
        AKPickerItemModel *model = [self.dataSource pickerView:self modelForItem:indexPath.item];
        
        if (itemModel.backgroundWidth > 0) {
            size.width += itemModel.backgroundWidth;
        }else{
            size.width += [self sizeForString:model.title].width;
        }
...
-(void)layoutSubviews {
    ...
    CGFloat backgroundViewWidth = self.backgroundViewWidth;
    self.backgroundView.frame = CGRectMake((self.contentView.bounds.size.width - backgroundViewWidth) / 2,
